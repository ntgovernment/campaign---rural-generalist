...
function initMmenu() {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (mmenuWrapper.getAttribute("data-btn-link") && mmenuWrapper.getAttribute("data-btn-text")) {
            btnContent = `<a role="button" class="btn-mmenu" href="${mmenuWrapper.getAttribute("data-btn-link")}">${mmenuWrapper.getAttribute("data-btn-text")}</a>`;
        ...
        close.innerHTML = `<div class="mm-btn--close">
                            ...
                            <span class="mm-btn--close__text">Menu</span>`;
        ...
        function toggleMapRegion(e) {
            ...
            if (dataMapRegion) {
                allGroupsNT.forEach(region => {
                    var text = region.querySelector("#" + region.id + "_text");

                    if (region.id == dataMapRegion) {
                        // Fill active colour if the tab references to the region
                        region.querySelectorAll("path").forEach(path => {
                            if (!path.classList.contains("exclude")) {
                                path.setAttribute("fill", "#E75D24");
                            }
                        });

                        fadeIn(text);
                    } else {
                        // Fill non-active colour
                        region.querySelectorAll("path").forEach(path => {
                            if (!path.classList.contains("exclude")) {
                                path.setAttribute("fill", "#FFCD00");
                            }
                        });
...
// sets aria-hidden="true" on h1s hidden above and below breakpoint 576px
function initAccessibleHeading() {
    var banner = document.querySelector('.ntg-banner');
    if (!banner) { return false; }
    var h1s = banner.querySelectorAll('h1');
    
    if (h1s.length > 1) {
        toggleAriaHidden(h1s);
    ...
    function toggleAriaHidden(elems) {
        if (window.matchMedia('(max-width: 576px)').matches) {
            for (let i = 0; i < elems.length; i++) {
                var elem = elems[i];
                
                if (elem.classList.contains('d-sm-none')) {
...
function initModals() {
    var triggers = document.querySelectorAll('a[href*="popup"]');
    if (!triggers) { return false; }

    triggers.forEach((e) => {
        e.setAttribute('data-bs-toggle', 'modal');
        var href = e.getAttribute('href');
        e.setAttribute('data-bs-target', href);
    });
}